...
let playerOldX, playerWidth, playerSpeed, playerPoints, bootPoints;
playerPoints = 0;
bootPoints = 0;
playerSpeed = 0.3;
playerWidth = 3;
...
// Load sound effects
const paddleHitSound = new THREE.Audio(listener);
...
audioLoader.load('sounds/paddle_hit.wav', function(buffer) {
  paddleHitSound.setBuffer(buffer);
  paddleHitSound.setVolume(0.5);  // Adjust volume
});

// Load boundary hit sound
audioLoader.load('sounds/boundary_hit.wav', function(buffer) {
    boundaryHitSound.setBuffer(buffer);
    boundaryHitSound.setVolume(0.5);
});

// Load background music
audioLoader.load('sounds/playBG.wav', function(buffer) {
...
function ballDetection() {
    ...
    if (ball.position.z <= player1.position.z && ball.position.z >= player1.position.z - (paddleSize.z )
    ...
    else if (ball.position.z >= boot.position.z && ball.position.z <= boot.position.z + (paddleSize.z )
    ...
    //     // }
    // }
    if (ballBB.intersectsBox(WallLBB) || ballBB.intersectsBox(WallL1BB)
        || ballBB.intersectsBox(WallRBB) || ballBB.intersectsBox(WallR1BB))
    {
        console.log("TOK");
        // if (keys.ArrowLeft)
        //     velocity
